...
const GalleryItemCard: React.FC<{ item: GalleryItem; onRemove: (id: string) => void }> = ({ item, onRemove }) => {
    return (
        <div className="group relative bg-gray-800/50 border border-gray-700 rounded-lg shadow-lg overflow-hidden aspect-video">
            <img 
                src={item.mediaUrl} 
                alt={`Contenido generado para: ${item.originalPrompt}`} 
                className="w-full h-full object-cover group-hover:opacity-50 transition-opacity bg-black"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent p-4 flex flex-col justify-end">
                <p className="text-white text-sm font-semibold line-clamp-2">{item.originalPrompt}</p>
                <p className="text-xs text-gray-400">{new Date(item.createdAt).toLocaleDateString('es-ES')}</p>
            </div>
    ...
    if (items.length === 0) {
        return (
            <div className="text-center py-12">
                <h2 className="text-2xl font-bold text-gray-200">Tu Galería está Vacía</h2>
                <p className="mt-2 text-gray-400">
                    ¡Ve al generador para crear y guardar tu primera creación con IA!
                </p>
            </div>
        );
    }
