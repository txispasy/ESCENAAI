...
export const PromptForm: React.FC<PromptFormProps> = ({ prompts, setPrompts, negativePrompt, setNegativePrompt, style, setStyle, aspectRatio, setAspectRatio, generationMode, setGenerationMode, uiMode, setUiMode, onSubmit, isLoading, cooldownEndTime }) => {
    ...
    const getButtonText = () => {
      ...
      if (cooldownSeconds > 0) {
        return `Ready in ${cooldownSeconds}s`;
      }
      ...
      return `Generate ${activePrompts} Scenes`;
    };
                ...
                {prompts.map((prompt, index) => (
                    <div key={index} className="relative">
                        <textarea
                            ...
                            placeholder={index === 0 ? "e.g., A cinematic shot of a raccoon in a library..." : "Add another scene description..."}
                            rows={2}
                        ...
                        {prompts.length > 1 && (
                            <button type="button" onClick={() => removePrompt(index)} className="absolute top-1/2 -translate-y-1/2 right-2 p-1.5 text-gray-500 hover:text-white rounded-full bg-gray-700/50 hover:bg-gray-600/50 transition-colors" aria-label="Remove prompt">
            ...
            <div className="space-y-4">
                ...
                {showAdvanced && (
                    <div id="advanced-settings" className="space-y-6 pt-4 border-t border-gray-700/50 animate-fade-in-down">
                        <div>
                            <label htmlFor="negative-prompt" className="block text-sm font-medium text-gray-300">Negative Prompt</label>
                            <textarea
                                id="negative-prompt"
